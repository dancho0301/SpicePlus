class ArticlesController < ApplicationController
  ...
  # 記事一覧
  def index
    @articles = @search.result.order("publication_date DESC").where("publication_date <= ? and publication = ?", Date.today, true)
  end

  # 記事
  # サイドバーには表示中の記事以外を表示する
  def show
    @all_articles = Article.where.not(id: params[:id]).order("publication_date DESC").where("publication_date <= ? and publication = ?", Date.today, true)
  end
